...
<body>
	...
	<form:form action="${pageContext.request.contextPath}/dangerlog/modify"
		method="post">
			...
			<tbody>
				...
				<tr>
					...
					<td style="padding: 6px 12px;"><fmt:formatDate
							value="${dangerLog.dangerTime}" pattern="yyyy-MM-dd HH:mm:ss" />
					</td>
				</tr>
				<tr>
					<th style="padding: 6px 12px; background-color: #f9f9f9;">드론
					...
					<td style="padding: 12px;">
						...
						<img
						src="${pageContext.request.contextPath}/dangerlog/getDangerSnapshot?danlogId=${dangerLog.danlogId}"
				...
				<tr>
					<th style="padding: 6px 12px; background-color: #eef7ff;">관제원
						현장 조치</th>
					<td style="padding: 12px;"><select name="dactionStatus"
						style="width: 200px; padding: 3px;">
							<option value="0"
								${dangerLog.dactionStatus == '0' ? 'selected' : ''}>미확인
								(0)</option>
							<option value="1"
								${dangerLog.dactionStatus == '1' ? 'selected' : ''}>조치중
								(1)</option>
							<option value="2"
								${dangerLog.dactionStatus == '2' ? 'selected' : ''}>조치완료
								(2)</option>
					</select></td>
				</tr>
				<tr>
					<th style="padding: 6px 12px; background-color: #eef7ff;">현장
						조치 사유/내용</th>
					<td style="padding: 12px;"><textarea name="dactionReason"
							rows="5" cols="60"
							placeholder="출동 요청, 상황 전파 등 구체적인 조치 이력을 기록하세요."
							required="required"><c:out
								value="${dangerLog.dactionReason}" /></textarea></td>
				</tr>
			</tbody>
		</table>
