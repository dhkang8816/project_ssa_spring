...
<body>
	...
	<div style="margin-bottom: 10px;">
		<button type="button"
			onclick="location.href='${pageContext.request.contextPath}/animal/list'">동물
			관리 목록</button>
	</div>
				...
				<c:otherwise>
					<c:forEach var="counter" items="${counterList}">
						<tr>
							...
							<td><fmt:formatDate value="${counter.lastUpdate}"
		...
		<ul class="pagination"
			...
			<c:if test="${pageMaker.prev}">
				<li style="margin-right: 5px;"><a
					href="counterList?page=${pageMaker.startPage - 1}&searchType=${pageMaker.searchType}&keyword=${pageMaker.keyword}">&laquo;
						이전</a></li>
			</c:if>
			...
			<c:forEach var="pageNum" begin="${pageMaker.startPage}"
				...
				<li class="${pageMaker.page == pageNum ? 'active' : ''}"
					style="margin-right: 5px;"><c:choose>
						...
						<c:otherwise>
							<a
								href="counterList?page=${pageNum}&searchType=${pageMaker.searchType}&keyword=${pageMaker.keyword}">${pageNum}</a>
						</c:otherwise>
					</c:choose></li>
			</c:forEach>
			...
			<c:if test="${pageMaker.next}">
				<li style="margin-right: 5px;"><a
					href="counterList?page=${pageMaker.endPage + 1}&searchType=${pageMaker.searchType}&keyword=${pageMaker.keyword}">다음
						&raquo;</a></li>
			</c:if>
		</ul>
	</div>
	...
	<form:form action="counterList" method="get">
		<select name="searchType">
			<option value="c" ${pageMaker.searchType == 'c' ? 'selected' : ''}>축종
				코드</option>
		</select>
