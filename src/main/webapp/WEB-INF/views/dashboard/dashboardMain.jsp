...
<head>
    ...
    <style>
		...
		.dashboard-container {
		    ...
		    margin-left: 260px !important; /* 💡 메뉴바 가로 크기에 맞춰 조절 (현재 약 250~260px 추정) */
...
<div class="dashboard-container">
    <div class="header-title">
        <i class="fa-solid fa-chart-pie"></i> 유기동물 AI 통합 관제 대시보드
    </div>
 ...
 <div class="ai-briefing-panel">
     <div class="panel-header">
         ...
         <button type="button" class="btn-refresh-ai" onclick="fn_fetchAiBriefing()">
     ...
     <div id="dashboardGraphZone" style="width: 100%; margin-top: 25px; display: none;">
         <div style="width:100%; display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
             <!-- 좌측 그래프룸 -->
             <div style="background:#222733; padding:20px; border-radius:8px; border:1px solid #2c313d;">
                 <div style="font-size:14px; color:#ffffff; font-weight:600; margin-bottom:15px;">
                     <i class="fa-solid fa-calendar-days" style="color:#5ddcff; margin-right:6px;"></i> 최근 7일간 일별 트렌드
                 </div>
                 <div style="position:relative; width:100%; height:250px;"><canvas id="trendChart"></canvas></div>
             </div>
             <!-- 우측 그래프룸 -->
             <div style="background:#222733; padding:20px; border-radius:8px; border:1px solid #2c313d;">
                 <div style="font-size:14px; color:#ffffff; font-weight:600; margin-bottom:15px;">
                     <i class="fa-solid fa-clock" style="color:#ffae19; margin-right:6px;"></i> 실시간 당일 시간대별 통계
                 </div>
                 <div style="position:relative; width:100%; height:250px;"><canvas id="timeChart"></canvas></div>
             </div>
         </div>
     </div>
     
     <!-- [블록 3] 축종별 / 이상객체별 분포 차트 공간 (기존 그래프존 바로 아래 추가 또는 내부에 배치) -->
	<div style="width:100%; display:grid; grid-template-columns: 1fr 1fr; gap:20px; margin-top:25px;">
	    
	    <!-- 좌측: 축종별 미달 경보 비율 -->
	    <div style="background:#222733; padding:20px; border-radius:8px; border:1px solid #2c313d; min-width: 0;">
	        <div style="font-size:14px; color:#ffffff; font-weight:600; margin-bottom:15px;">
	            <i class="fa-solid fa-dog" style="color:#2ecc71; margin-right:6px;"></i> 축종별 경보 발생 분포
	        </div>
	        <div style="position:relative; width:100%; height:250px;"><canvas id="animalChart"></canvas></div>
	    </div>
	
	    <!-- 우측: 이상객체 종류별 포착 현황 -->
	    <div style="background:#222733; padding:20px; border-radius:8px; border:1px solid #2c313d; min-width: 0;">
	        <div style="font-size:14px; color:#ffffff; font-weight:600; margin-bottom:15px;">
	            <i class="fa-solid fa-skull-crossbones" style="color:#e74c3c; margin-right:6px;"></i> 이상객체 유형별 포착 통계
	        </div>
	        <div style="position:relative; width:100%; height:250px;"><canvas id="dangerTypeChart"></canvas></div>
	    </div>
	        ...
	        $("#dashboardGraphZone").show();
                ...
                window.myTimeChart = new Chart(ctxTime, {
                    ...
                    data: {
                        ...
                        datasets: [
                            { label: '위험객체 (회)', data: res.dangerTimeData, backgroundColor: '#e74c3c', borderRadius: 4 },
                            { label: '미달경보 (건)', data: res.detectTimeData, backgroundColor: '#f1c40f', borderRadius: 4 }
                        ]
                    },
                    options: { 
                        responsive: true, 
