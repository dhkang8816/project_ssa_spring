			...
			<div id="util-box">
				...
				<c:if test="${not empty sessionScope.SPRING_SECURITY_CONTEXT}">
					<div class="user-info-box">
						...
						<!-- 내 정보 보기 프로필 이미지 -->
						<a
							href="${pageContext.request.contextPath}/member/detail?memberId=${sessionScope.SPRING_SECURITY_CONTEXT.authentication.principal}"
							class="header-profile-link" title="내 정보 보기"> <img
							src="${pageContext.request.contextPath}/resources/images/member/noImage.jpg"
							class="header-profile-img" alt="프로필">
						</a>

						<!-- 사번 텍스트 -->
						<span class="user-text-info"> <strong>${sessionScope.SPRING_SECURITY_CONTEXT.authentication.principal}</strong>님
							로그인 중
						</span>
						...
						<div class="alarm-container"
							style="position: relative; margin-left: 15px; display: inline-block; vertical-align: middle;">
							<!-- 꽉 찬 종 모양 아이콘 -->
							<i class="fa-solid fa-bell" id="alarmBellIcon"
								style="font-size: 20px; color: #ffffff; cursor: pointer;"></i>

							<!-- 알림 빨간 배지 -->
							<span class="alarm-count-badge"
								style="position: absolute; top: -5px; right: -5px; background-color: #ff4d4d; color: white; font-size: 10px; padding: 2px 5px; border-radius: 50%; font-weight: bold; display: none;">0</span>

							<!-- 클릭 시 아래로 무조건 튀어나오는 실시간 알림 팝업창 -->
							<div class="alarm-dropdown"
								style="display: none; position: absolute; top: 35px; right: -10px; width: 320px; max-height: 400px; background-color: #ffffff; border: 1px solid #dddddd; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.2); border-radius: 8px; z-index: 99999; overflow-y: auto;">

								<!-- 팝업 헤더 -->
								<div
									style="padding: 12px 15px; border-bottom: 1px solid #eee; background-color: #f8f9fa; font-weight: bold; color: #333; font-size: 14px;">
									📢 실시간 시스템 경보 알림</div>

								<!-- 팝업 내부 목록 리스트 레이어 -->
								<ul class="alarm-list-content"
									style="list-style: none; padding: 0; margin: 0; font-size: 13px; color: #555;">
									<c:if test="${empty headerAlertList}">
										<li class="empty-alarm-msg"
											style="padding: 25px; text-align: center; color: #999;">새로운
											경보 알림이 없습니다.</li>
									</c:if>
									<c:if test="${not empty headerAlertList}">
										<c:forEach var="alert" items="${headerAlertList}">
											<li
												style="padding: 12px 15px; border-bottom: 1px solid #f5f5f5; line-height: 1.4; background-color: #ffffff;">
												...
												<div
													style="font-size: 11px; color: #aaa; margin-top: 4px; text-align: right;">
													<fmt:formatDate value="${alert.firstSendTime}"
														pattern="MM-dd HH:mm" />
												</div>
											</li>
										</c:forEach>
								...
								<!-- 팝업 푸터 -->
								<div
									style="padding: 10px; text-align: center; border-top: 1px solid #eee; background-color: #f8f9fa; border-bottom-left-radius: 8px; border-bottom-right-radius: 8px;">
									<a href="${pageContext.request.contextPath}/alert/list"
										style="text-decoration: none; color: #007bff; font-size: 12px; font-weight: bold;">전체
										이력 보기</a>
								</div>
							</div>
						</div>
					...
					<!-- 👑 2. [위치 수정] 유저 박스를 닫는 </div>가 알림창 전체 구조가 완전히 끝난 이 자리로 내려옵니다! -->
				</c:if>
