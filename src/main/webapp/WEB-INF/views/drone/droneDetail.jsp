	...
	<form:form id="droneForm" method="post">
			...
			<tbody>
				<tr>
					...
					<td style="padding: 5px 10px;">
						<!-- 문자열 식별키(PK)이므로 수정을 방지하기 위해 readonly 처리 --> <input type="text"
						name="droneId" value="${drone.droneId}" readonly="readonly"
						style="background-color: #eee; width: 250px;" />
					</td>
				</tr>
				<tr>
					<th style="padding: 5px 10px;">담당 관제원 변경</th>
					<td style="padding: 5px 10px;">
						<!-- 💡 기존 배정된 사람을 자동으로 select 해주는 가변 목록 구조 --> <select
						name="memberId" style="width: 256px;">
							<option value="">-- 담당 관제원 선택 (미배정) --</option>
							<c:forEach var="member" items="${memberList}">
								<option value="${member.memberId}"
									${drone.memberId == member.memberId ? 'selected="selected"' : ''}>
									${member.name} (${member.memberId})</option>
							</c:forEach>
					</select>
					</td>
				</tr>
		...
		<div>
			...
			<button type="button" onclick="fn_submit('remove')"
				style="background-color: #f44336; color: white;">기체 삭제</button>
